...
url="git.oschina.net/renhao/renhao/raw/master/amh-4.2/Module/file";
...
[ ! -f 'dkms-2.2.0.3-1.noarch.rpm' ] && wget http://linux.dell.com/dkms/permalink/dkms-2.2.0.3-1.noarch.rpm;
[ ! -f 'kernel_ppp_mppe-1.0.2-3dkms.noarch.rpm' ] && wget http://${url}/kernel_ppp_mppe-1.0.2-3dkms.noarch.rpm;
[ ! -f "${ppp_name}.${SysBit}.rpm" ] && wget http://${url}/${ppp_name}.${SysBit}.rpm;
[ ! -f "${pptpd_name}.${SysBit}.rpm" ] && wget http://${url}/${pptpd_name}.${SysBit}.rpm;
...
echo "vpn pptpd vpn *" >> /etc/ppp/chap-secrets
