...
function ConfirmInstall()
{
	...
	select selected in 'Install AMH ++' 'Uninstall AMH ++' 'Exit'; do break; done;
	[ "$selected" == 'Exit' ] && echo 'Exit Install.' && exit;

	if [ "$selected" == 'Install AMH ++' ]; then
		InstallModel='1';
	elif [ "$selected" == 'Uninstall AMH ++' ]; then
		Uninstall;
	else
		ConfirmInstall;
		return;
...
{
	read -p '[Notice] Please input AMH/MYSQL password:' AMHPass;
	if [ "$AMHPass" == '' ]; then
		echo '[Error] AMH/MYSQL password empty.';
		InputAMHPass;
	else
		echo '[OK] Your AMH/MYSQL password is:';
		echo $AMHPass;
	fi;
}
...
# **************************************
		...
		/usr/local/mysql/bin/mysqladmin password $AMHPass;
		rm -rf /usr/local/mysql/data/test;

# EOF **********************************
mysql -hlocalhost -uroot -p$AMHPass <<EOF
USE mysql;
DELETE FROM user WHERE User!='root' OR (User = 'root' AND Host != 'localhost');
UPDATE user set password=password('$AMHPass') WHERE User='root';
DROP USER ''@'%';
FLUSH PRIVILEGES;
EOF
# **************************************
...
function InstallNginx()
{
	...
	if [ ! -d /usr/local/nginx ]; then
		...
		./configure --prefix=/usr/local/nginx --user=www --group=www --with-http_ssl_module  --with-http_gzip_static_module --with-http_spdy_module --with-http_spdy_module --without-mail_pop3_module --without-mail_imap_module --without-mail_smtp_module --without-http_uwsgi_module --without-http_scgi_module ;
		make -j $Cpunum;
		make install;
...
function InstallAMH()
{
	...
	if [ ! -d /home/wwwroot/index/web ]; then
		...
		SedMysqlPass=${AMHPass//&/\\\&};
		SedMysqlPass=${SedAMHPass//\'/\\\\\'};
		sed -i "s/'AMHPass'/'${SedAMHPass}'/g" /home/wwwroot/index/web/Amysql/Config.php;
		...
		/usr/local/mysql/bin/mysql -u root -p$AMHPass < $AMHDir/conf/amh.sql;
...
rm -rf $AMHDir;
# endlogo 	******************************************************************
      echo 'Thank you for using AMH ++';
	  echo '[AMH] Congratulations, AMH ++ install completed.';
