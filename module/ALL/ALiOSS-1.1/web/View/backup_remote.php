...
<table border="0" cellspacing="1"  id="STable" style="width:1080px;">
	...
	?>
			...
			<a href="index.php?c=backup&a=backup_list&category=backup_remote&check=<?php echo $val['remote_id'];?>" class="button" onclick="return connect_check(this);"><span class="loop icon"></span>连接测试</a>
			<a href="index.php?c=backup&a=backup_list&category=backup_remote&edit=<?php echo $val['remote_id'];?>" class="button"><span class="pen icon"></span>编辑</a>
			<a href="index.php?c=backup&a=backup_list&category=backup_remote&del=<?php echo $val['remote_id'];?>" class="button" onclick="return confirm('确认删除远程备份设置ID:<?php echo $val['remote_id'];?>?');"><span class="cross icon"></span>删除</a>
			<?php } else { ?>
...
<form action="index.php?c=backup&a=backup_list&category=backup_remote" method="POST"  id="remote_edit" onsubmit="return remote_pass_type_fun(G('remote_pass_type_dom'));"/>
